...
def truncate_llm_response(response: str, head_tail_length: int = 100) -> str:
    """Truncate LLM response for debug logging, showing both start and end of the response"""
    if not response:
        return response
        
    if len(response) <= head_tail_length * 2:
        return response
        
    head = response[:head_tail_length]
    tail = response[-head_tail_length:]
    middle_length = len(response) - (head_tail_length * 2)
